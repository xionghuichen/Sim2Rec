...
if __name__ == '__main__':
    ...
    vae = VAE(args.hid_size, args.hid_layers, args.layer_norm, args.constant_z_scale, args.lr, 
              args.res_struc, [6], args.z_size, np.zeros((55, 6)), sess, 'vae', l2_reg_coeff=args.l2_reg_coeff)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if epoch % 100 == 0 and epoch >= 0:
            recons_data_all = []
            for idx in range(test_data_dict["4245"].shape[0]):
                test_code, recons_data = vae.reconstruct_samples(test_data_dict["4245"][idx], num_product) #4245
                recons_data_all.append(recons_data)
            print('recons_data_all.shape', recons_data_all.shape)
            ...
            print('recons_data[:, 3]', recons_data_all[:,:,3]) #3: price 5:sales
            kld = compute_kl(real_action_mean, real_action_std, recons_data_all[:,:,3])
