...
from vae_lts.src.vae_handler_multi import VAE
...
def compute_kl(real_mean, real_std, recons_data):
    print('real_mean.shape', real_mean.shape)
    print('real_std.shape', real_std.shape)
    ...
    print('real_logstd.shape', real_logstd.shape)
    print('recons_logstd.shape', recons_logstd.shape)
    print('recons_std.shape', recons_std.shape)
    print('recons_mean.shape', recons_mean.shape)
...
if __name__ == '__main__':
    ...
    for store, state in store_state.items():
        if state in args.train_state_list:
            ...
            data_dict[store] = vae_data_info_preprocess(store_env)
    ...
    for store, state in store_state.items():
        if state in args.test_state_list:
            ...
            store_env = SingleStoreEnvironment(store, store_model)
            env_dict[store] = store_env
            test_data_dict[store] = vae_data_info_preprocess(store_env)
    ...
    vae = VAE(args.hid_size, args.hid_layers, args.layer_norm, args.constant_z_scale, args.lr, 
              args.res_struc, [50 * 6], args.z_size, np.zeros((50 * 6)), sess, 'vae', l2_reg_coeff=args.l2_reg_coeff)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if epoch % 100 == 0 and epoch >= 0:
            test_code, recons_data = vae.reconstruct_samples(test_data_dict["4245"].reshape((-1, 50*6)), test_data_dict["4245"].shape[0]) #4245
            recons_data = recons_data.reshape((-1, 50, 6))
            print('recons_data.shape', recons_data.shape)
        ...
        for train_store in train_store_list:
            ...
            data_batch = data_batch[:,:50,:].reshape((-1, 50*6))
