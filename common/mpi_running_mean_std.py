...
class RunningMeanStdNp(object):
    def __init__(self, epsilon=1e-2, shape=()):
        ...
        """
        self._sum = np.zeros(shape=shape, dtype=np.float32)
        self._sumsquare = np.ones(shape=shape, dtype=np.float32) * epsilon
        self._count = np.zeros(shape=shape, dtype=np.float32)
        self.shape = shape
    ...
    def update(self, data):
        ...
        self._sum += addvec[0]
        self._sumsquare += addvec[1]
        self._count += addvec[2]
    ...
    @property
    def std(self):
        return np.sqrt(np.maximum(self._sumsquare / self._count - np.square(self.mean), 1e-2))
