...
from common.logger import read_json, read_csv
...
def load_results(root_dir_or_dirs, names, x_bound, enable_progress=True, enable_monitor=True, use_buf=False, verbose=False):
    '''
                    ...
                    if osp.exists(progjson):
                        ...
                        try:
                            ...
                            if osp.exists(buf_csv) and use_buf:
                                ...
                                reader = pd.read_csv(progcsv, chunksize=100000,  quoting=csv.QUOTE_NONE, encoding='utf-8', index_col=False, comment='#')
                                raw_df = pd.DataFrame()
                                for chunk in reader:
                                    slim_chunk = chunk
...
def plot_results(
    ...
    for (isplit, sk) in enumerate(sorted(sk2r.keys())):
        ...
        if average_group:
            for group_id, group in enumerate(sorted(groups)):
                ...
                if resample:
                    ...
                    ys = []
                    for (x, y) in xys:
                        if rescale_list is not None:
                            scale = rescale_list[group_id]
                        else:
                            scale = 1
                        ys.append(scale * symmetric_ema(x, y, low, high, resample, decay_steps=smooth_step)[1])
                else:
        ...
        if any(g2l.keys()):
            ...
            if replace_legend_keys_1to1 is not None:
                # new version
                legend_keys = np.array(replace_legend_keys_1to1)
            ...
            if not skip_legend:
                lgd = ax.legend(
                    ...
                    fontsize=16 if pretty else None)
            else:
                lgd = None
        ax.set_title(sk)
        if log:
