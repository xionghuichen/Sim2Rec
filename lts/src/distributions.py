...
class DiagGaussianProbabilityDistributionType(ProbabilityDistributionType):
    ...
    def proba_distribution_from_latent(self, pi_latent_vector, vf_latent_vector, init_scale=1.0, init_bias=0.0, act_fn=tf.identity,
                                       learn_var=False):
        mean = act_fn(linear(pi_latent_vector, 'pi', self.size, init_scale=init_scale, init_bias=init_bias))
        if not learn_var:
            std = tf.get_variable(name='pi/logstd', shape=[1, self.size], initializer=tf.ones_initializer())
            logstd = tf.log(std)
            pdparam = tf.concat([mean, mean * 0.0 + logstd], axis=1)
        else:
            logstd = tf.log(tf.nn.sigmoid(linear(pi_latent_vector, 'pi_log', self.size, init_scale=init_scale, init_bias=init_bias)) + 1e-6)
            pdparam = tf.concat([mean, logstd], axis=1)
        q_values = linear(vf_latent_vector, 'q', self.size, init_scale=1.0, init_bias=init_bias)
